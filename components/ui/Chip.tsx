  ...
  return (
    <ul className={cx('flex flex-wrap items-center justify-center gap-[10px]', className)}>
      {chips.slice(0, 5).map((chip) => {
        const inner = (
          <span className="block rounded-badge border border-line px-[14px] py-[8px] text-s text-muted transition-colors duration-ui ease-ui hover:border-ink hover:text-ink">
            {chip.label}
          </span>
        )
        return (
          <li key={chip.label}>
            {chip.href ? (
              <a href={chip.href} className="block min-h-[44px] content-center">
                {inner}
              </a>
            ) : (
              inner
